# 🌿 WildScout — Market Opportunity

## 🚨 The Problem

AllTrails has **60M+ users** who go outside to hike, but the experience ends at the trail.  
There’s no equivalent app that tells you **what wildlife and plants you’ll encounter along the way**.

Meanwhile, **iNaturalist** has an incredible biodiversity database, but it’s built for:
- scientists
- hardcore naturalists  

—not casual outdoor users.

### 🔎 The Gap

> **AllTrails tells you where to go.  
iNaturalist tells you what’s there.  
Nobody combines the two.**

---

## 😕 Pain Points for Outdoor Users

- ❌ Don’t know what species are in their area without deep research  
- ❌ Can’t plan hikes around seasonal wildlife (wildflowers, migrations, etc.)  
- ❌ No easy way to discover biodiversity hotspots like you would trails  

---

## 🏔️ AllTrails (The Competitor)

- 60M+ registered users  
- 1M+ paying subscribers  
- $37.9M revenue in 2023 (up from $5.5M in 2021)  
- $151M raised, 383 employees  
- Users spent **75% more time outdoors in 2025 vs 2024**

---

## 🌎 iNaturalist (The Data Source)

- 250M+ observations  
- ~4M contributors, 400K+ identifiers  
- 4.5M monthly users (but only ~5% contribute data)  
- Massive untapped casual audience  
- Research usage has grown **10x in 5 years**

---

## 💰 The Outdoor Recreation Economy

- $696.7B contribution to U.S. GDP (2024)  
- $1.3T total economic output  
- 5.2M jobs  
- Nearly:
  - 4× larger than air transportation  
  - 2.5× larger than agriculture/forestry  

---

## 🦅 Wildlife & Nature Tourism

- $190.7B global market (2025) → projected **$381B by 2033**  
- Birdwatching alone = **$70B market**  
- 96M Americans participate in birdwatching  

---

# 📊 Market Sizing (TAM / SAM / SOM)

## 🌍 TAM (Total Addressable Market)
**~$190B**

Global wildlife tourism:
- safaris  
- birdwatching  
- ecotourism  
- nature photography  
- outdoor recreation  

---

## 📱 SAM (Serviceable Available Market)
**~$2B – $5B**

Digital outdoor + nature apps:
- AllTrails users (60M+)  
- iNaturalist audience (~4.5M/month)  
- Tech-enabled nature tourism  

💡 Example:
If even a small % of the **96M birdwatchers** pay $3–5/month → **billions in revenue potential**

---

## 🚀 SOM (Serviceable Obtainable Market)
**~$10M – $50M (Years 1–3)**

Initial target:
- Nature-curious hikers  
- Existing AllTrails + iNaturalist users  
- U.S. regions:
  - California  
  - Pacific Northwest  
  - Southeast  

### 📈 Example Growth Model

- 500K users (Year 1)  
- 2% conversion → 10K paying users  
- $30/year subscription  

👉 **~$300K ARR starting point**

---

## 🔥 Scaling Opportunities

- Species checklists  
- Seasonal wildlife alerts  
- Guided wildlife experiences  
- Advanced recommendations  

---

# 🌟 Why WildScout Wins

WildScout bridges the gap between:
- 🗺️ **Where to go (AllTrails)**  
- 🌿 **What you’ll see (iNaturalist)**  

> Turning passive hikes into **data-driven nature experiences**
